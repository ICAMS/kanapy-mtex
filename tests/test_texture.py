...
import numpy as np
import pytest
from kanapy_mtex import __backend__, MTEX_DIR

@pytest.mark.skipif(not __backend__=='mtex', reason="Kanapy is not configured for texture analysis yet!")
def test_mex():
...
def test_readEBSD():
    ...
    assert np.abs(gs_param[0] - 0.64620215) < 1.e-4
    # get list of orientations for grains in RVE matching the ODF of the EBSD map
    ori_rve = ebsd.calcORI(20)
    assert np.abs(ori_rve[0, 1] - 0.5235987755982987) < 1.e-5

@pytest.mark.skipif(not  __backend__=='mtex', reason="Kanapy is not configured for texture analysis yet!")
def test_createORI():
    from kanapy_mtex.texture import createOriset
    Ngr = 10
